...
class Main(QMainWindow):
    def __init__(self, user_id: int, role: str, clinics_csv: str, username: str):
        ...
        ok = fail = 0
        try: ok, fail = sync_buffer_once(self.conn)
        except Exception as e: QMessageBox.information(self, "Start-Sync", f"Start-Sync nicht abgeschlossen:\n{e}")
        sb_msg = f"Start-Sync: {ok} synchronisiert" + (f", {fail} offen" if fail else "")

        tb = QToolBar("Aktionen"); self.addToolBar(tb)
        act_export_open = QAction("Offene exportieren", self); act_export_open.triggered.connect(self.export_open)
        act_export_done = QAction("Erledigte exportieren", self); act_export_done.triggered.connect(self.export_done)
        tb.addAction(act_export_open); tb.addAction(act_export_done)

        self.tabs = QTabWidget()

        def on_clinics_changed_cb():
            if hasattr(self, "tab_create"): self.tab_create._reload_clinics()
        ...
        if self.role != "Viewer":
            self.tab_create = CreateTab(self.conn, role=self.role, clinics_csv=self.clinics_csv, submitter_default=self.username, current_username=self.username)
            self.tab_create.case_created.connect(self._on_case_created)
            self.tabs.addTab(self.tab_create, "Erfassen")

        self.tab_open = OpenTab(self.conn, role=self.role, clinics_csv=self.clinics_csv, read_only=(self.role == "Viewer"), current_username=self.username)
        ...
        apply_system_theme()
        try: QApplication.styleHints().colorSchemeChanged.connect(apply_system_theme)
    ...
    def export_done(self):
        ...
        with open(path, "w", newline="", encoding="utf-8") as f:
            w = csv.writer(f, delimiter=";")
            w.writerow(["ID","Klinik","Gerät","Wave- / Serienummer","Abgeber","Techniker","Grund","Abgabe","Zurück"])
            w.writerows(rows)
        QMessageBox.information(self, "Export", "Export abgeschlossen.")

def run():
    app = QApplication(sys.argv); apply_app_theme(app)
    try: app.styleHints().colorSchemeChanged.connect(lambda: apply_app_theme(app))
    except Exception: pass

    login = Login(); login.show(); app.exec()
    if getattr(login, 'authed', None):
        uid, role, clinics_csv = login.authed; uname = login.user.text().strip()
        win = Main(uid, role, clinics_csv, uname); win.show()
        sys.exit(app.exec())

...
class CreateTab(QWidget):
    ...
    def __init__(
        ...
        if submitter_default: self.submitter.setText(submitter_default[:MAX_INPUT_CHARS])
        ...
        self.notes = QTextEdit(); self.notes.setPlaceholderText(f"Notizen (max. {MAX_INPUT_CHARS} Zeichen)")
        self.notes.textChanged.connect(self._enforce_notes_limit)  # Live-Limit (auch bei Paste)

        # --- Layout ---
        form = QFormLayout()
        form.addRow("Klinik*", self.clinic)
        form.addRow("Gerät*", self.device)
        form.addRow("Wave- / Serienummer*", self.wave)  # Label konsistent
    ...
    def _enforce_notes_limit(self):
        ...
        if len(txt) <= MAX_INPUT_CHARS: return
        cur = self.notes.textCursor(); pos = cur.position()
        ...
        cur.setPosition(pos); self.notes.setTextCursor(cur)
        self.notes.blockSignals(False)

    # ----------------- Persistenz -----------------
    def _ensure_columns(self):
        cur = self.conn.cursor(); cur.execute("PRAGMA table_info(cases);")
        existing = {row[1] for row in cur.fetchall()}
        with self.conn:
            if "created_by" not in existing: self.conn.execute("ALTER TABLE cases ADD COLUMN created_by TEXT")
            if "closed_by"  not in existing: self.conn.execute("ALTER TABLE cases ADD COLUMN closed_by  TEXT")

    # ----------------- Aktionen -----------------
    def on_save(self):
        # Pflichtfelder (defensiv erneut kappen)
        ...
        if notes: notes = notes[:MAX_INPUT_CHARS]

        payload = {
            "clinic": clinic, "device_name": device, "wave_number": wave or None,
            "submitter": submitter or None, "service_provider": provider or None,
            "status": "In Reparatur", "reason": reason or None,
            "date_submitted": date_submitted_str, "date_returned": None,
            "notes": notes, "created_by": self.current_username or None,
        }
        ...
        try:
            ...
            self._clear_form(); QMessageBox.information(self, "Erfasst", "Fall wurde erfasst (Status: In Reparatur).")
            self.case_created.emit()
        except Exception:
            enqueue_write(dict(payload, type="insert_case"))
            self._clear_form()
            QMessageBox.information(self, "Offline gespeichert",
